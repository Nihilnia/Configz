...
function getranges(_, exists, db, more)
    ...
    local function callback(success, result, error)
        ...
        mp.msg.debug("Got: " .. string.gsub(stdout, "[\n\r]", ""))
        if not string.match(stdout, "^%s*(.*%S)") then return end
        if string.match(stdout, "error") then return getranges(true, true) end
        local new_ranges = {}
        local r_count = 0
    ...
    if not legacy then
        mp.command_native_async({name = "subprocess", capture_stdout = true, playback_only = false, args = args}, callback)
    else
        -- Legacy support remains blocking as it lacks a proper async subprocess with callback in older mpv versions
        local sponsors = utils.subprocess({args = args})
        callback(true, sponsors, nil)
    end
end
