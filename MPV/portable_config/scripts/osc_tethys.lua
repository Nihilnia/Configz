...
local tethys = {
    ...
    pipGeometry = "25%+-10+-10", -- PictureInPicture 33% screen width, 10px from bottom right
...
opt.read_options(tethys, "tethys")
...
function Thumbnailer:register_client()
    ...
    mp.add_key_binding(thumb_script_key, "generate-thumbnails", function()
        if self.state.available and thumbnailer_options.autogenerate then
            ...
            self:start_worker_jobs()
        elseif not thumbnailer_options.autogenerate then
            mp.osd_message("Thumbnails are disabled (press t to enable)")
        else
            mp.osd_message("Thumbnailing unavailable")
        end
    end)

    -- thumbnail-toggle: enable or disable thumbnail generation on the fly
    mp.register_script_message("thumbnail-toggle", function()
        thumbnailer_options.autogenerate = not thumbnailer_options.autogenerate
        if thumbnailer_options.autogenerate then
            -- Re-run state detection and kick off generation for the current file
            self:update_state()
            if self.state.available then
                self:start_worker_jobs()
            end
            mp.osd_message("Thumbnails: ON")
        else
            -- Wipe the in-memory thumbnail table so the hover tooltip goes blank
            self:clear_state()
            mp.osd_message("Thumbnails: OFF")
        end
    end)
end
...
function togglePictureInPicture()
    ...
    if isPiP then -- Disable
        ...
        if not tethys.pipWasFullscreen and not tethys.pipWasMaximized then
            -- Defer restore to let border/decoration changes settle first.
            -- The PiP geometry ("33%+-10+-10") leaves a screen-edge-relative anchor
            -- that causes window-scale to snap the window to the bottom when applied
            -- immediately. A deferred scale followed by a deferred pos-set breaks that.
            local savedScale = tethys.pipWasScale
            local savedX = math.floor(tethys.pipWasPosX)
            local savedY = math.floor(tethys.pipWasPosY)
            mp.add_timeout(0, function()
                mp.set_property('window-scale', tostring(savedScale))
                -- Defer position one more tick so the resize from scale has settled.
                -- Use geometry string (+X+Y) instead of window-pos-x/y to explicitly
                -- clear the screen-edge-relative anchor left by the PiP geometry
                -- ("33%+-10+-10"). window-pos-x/y cannot override that anchor reliably.
                mp.add_timeout(0, function()
                    mp.commandv('set', 'geometry', "50%:50%")
                end)
            end)
        end
        ...
        tethys.pipWasScale = tonumber(mp.get_property('window-scale')) or 1
        tethys.pipWasPosX = mp.get_property_number('window-pos-x') or 0
        tethys.pipWasPosY = mp.get_property_number('window-pos-y') or 0
...
mp.register_script_message("toggle-pip", togglePictureInPicture)